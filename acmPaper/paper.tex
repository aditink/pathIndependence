...
The proof \xxx[as]{which proof?} is not affected if any of these \xxx[as]{which?} paths is the identity, e.g., if $f_1$ is the identity and $S$ and $P_1$ are actually the same node.
\xxx[as]{Maybe this observation belongs somewhere else, where we are talking about the correctness argument/proof?}
