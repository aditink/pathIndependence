...
The key observation allowing us to improve on the online baseline is a result of Theorem~\ref{reductionRule} (which will be expanded on later): for a given source and sink pair, only a single parallel pair needs to be verified.
...
Algorithm \ref{algo_online_polynomial} uses this strategy of identifying a parallel pair with exactly one path through the edge for each (source, sink) pair.
...
\end{tikzpicture}
Each arrow represents a path, where $(S,T)$ is the new edge being added.
\end{center}
...
\xxx[as]{At some point, we have switched to capital letters for both paths and node variables... maybe make this consistent?} \xxx[dg]{I think these are all nodes, so it should be good}
...
\xxx[as]{Do we need an argument for why?} \xxx[dg]{Yeah, we probably do}
...
Ultimately verification is performed by calling an equality oracle (that is beyond the scope of this work) for every comparable pair returned by the previous stage, as in \ref{algo_verification}\xxx[as]{This algorithm can probably be summarized in one sentence and doesn't seem that helpful} .
...
Using this algorithm, we found a bug in the reflection model, where a matrix was not being divided correctly by its determinant \xxx[dg]{List other things I find, if anything}\xxx[ak]{I think it may be nice to add more detail for at least one bug (code, what was the program doing, what the graph/conflict was, time taken, etc.)}.  We also inserted previous bugs to check the algorithm was working correctly, and it identified all inserted bugs\xxx[as]{This general observation is less interesting than specific bugs, and should be omitted to just use the specifics}.
...
It does not appear to support the implicit definition of conversion between unites \xxx[AK]{Need to spend some time confirming this} but if extended to do so, would need to contend with the problem of commuting graphs.
...
Beyond implementation work, this requires an answer to the question of how to write function equality oracles.\xxx[as]{Could we say a sentence or two about why this would be useful?  Also maybe use Scala as an example of a broader languages with this property}
...
\begin{figure}
	...
	\begin{subfigure}{\linewidth}
		\includegraphics[width=\linewidth]{shadowmap.png}
		\caption{Shadow Map Model}
	\end{subfigure}
	...
	\caption{Example Images from Gator Programs}
	\label{fig:gator-images}
\end{figure}
