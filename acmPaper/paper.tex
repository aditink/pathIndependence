...
Such a type system is similar to existing type systems, such as that implemented in F\#~\cite{unitsOfMeasure} and a variety of C-like languages.
...
Here, $f_1 \circ f_2 \circ f_3$ is a two flip path, and $f_1 \circ (S, T) \circ f_3$ is a new path created because of the addition of ($S$,$T$) that forms a parallel pair with $f_2$.
...
\begin{itemize}
\item In the case where p has two flips, $t_1(p) \circ (S, T) \circ t_3(p)$ and $t_2(p)$ form a parallel pair.
\item When p has only the first flip (which is to say, the third phase of the path is missing), the parallel paths are $t_1(p) \circ (S, T)$ and $t_2(p)$.
\item Similarly when only the second flipping point is present (so that there is no first phase), then the parallel pair is  $(S, T) \circ t_3(p)$ and $t_2(p)$.
