...
\KwRet{parallelPairs}\;
\caption{Online polynomial time algorithm to find parallel pair set}\label{algo_online_polynomial}
\end{algorithm}
...
\caption{Minimal set finding algorithm}\label{algo_online_minimal}
\end{algorithm}
...
Given a graph and a new edge, we time how long it takes for an algorithm to return 
...
\section{Comparison of algorithm time cost}
The average time taken by each algorithm over the course of 10 runs over randomly generated graphs with 8 nodes and 32 edges is listed in table ~\ref{times}.
...
\begin{tabular}{|c|c|}
    ...
    \hline
    Naive baseline & 1,500 \\
    Two Flip tolerant & 1 \\
    Batch algorithm & 1.4 \\
    algorithm ~\ref{algo_online_polynomial} & 1.2 \\
    algorithm ~\ref{algo_online_minimal} & 0.0006 
    \hline
    \label{times}
\end{tabular}

\section{Scaling of time with input size}

Algorithms scale as expected with time, as seen in figure ~\ref{fig:timeVsSize}. 
Both algorithm ~\ref{algo_online_minimal} and ~\ref{algo_minimal_spanning_set} exhibit graphs that are polynomial in appearance, as does the batch baseline.
\xxx[AK]{Do I need to justify this by doing some kind of fit?}
The naive baseline as well as the two flip tolerant baseline display catastrophic growth that quickly makes computation untenable.

Further, run time is found to relate to the density of edges in the input graph.
...
Generally, time cost is found to be higher at middling densities and lower as density approaches 0 or 1.
...
\begin{figure}
    ...
    %TODO add plots for two flip, naive, batch checker
\end{figure}

\section{Variance}
For all algorithms, there is considerable spread in the time taken for a given graph size, varying with graph structure.
The plots in figure ~\ref{fig:timeVsSize} %TODO replace
depict the situation.
After computation on 10 randomly generated graphs of density 0.5, each outcome is plotted as a point and error bars indicate standard deviation.

%TODO add a figure for each of the checkers.

\section{Size of Output}
Output size is found to be along similar lines to time cost.
Table ~\ref{sizes}, the size analog of table ~\ref{times} summarizes the number of output pairs that the algorithms returned on average over 10 runs, for graphs with 8 nodes and 32 edges.
% TODO get this data.
\begin{tabular}{|c|c|}
    \hline
