...
\xxx[dg]{Give a summary of the algorithm though, breaking down important parts.  I think that's mostly been done above, but some actual code snippets might help}
\begin{verbatim}
Graph existingGraph;
R_s = {}
...
\xxx[dg]{Insert an introduction paragraph here about what we're solving before jumping into the details}

\xxx[dg]{As we've discussed, I think the existence of an identity function doesn't have to be assumed, since every node "exists" as itself.  I guess another way to put it is that we assume the 0-edge path exists as the "identity" function}
