...
Na\"{i}vely checking if all path pairs that begin and end at the same node in a given diagram commute could require a number of function equality checks that grow as factorial in the number of nodes, since a path consists of an ordering of nodes.
...
We compare our solution to three baseline implementations: a Na\"{i}ve checking of all path pairs with only special handling for cycles, a check for all path pairs that involve the new edge, and an algorithm suggested by previous work to solve the batch version of the problem for acyclic diagrams.
...
$\partial(p)^{+}$ is the start node, and $\partial(p)^{-}$ is end node).
